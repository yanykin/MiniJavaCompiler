/*
* –еализует удобный вывод построенного синтакстического дерева
*/
#pragma once
#include "Visitor.h"
...
public:
	...
	// ѕечатает заданное число отступов
	void PrintTabs( size_t tabs ) const;
	// ѕечатает отступ в соответствии с текущим уровнем вложени€
	void PrintMargin() const;
	// ”величивает и уменьшает отступ
...
private:
	static std::string marginString; // строка, котора€ будет печататьс€ как отступ
	size_t marginInTabs; // задаЄт уровень отступа, на котором нужно производить печать
};
